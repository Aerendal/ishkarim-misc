"""
test_misc_domain.py — testy domenowe dla ishkarim_misc.
...
from ishkarim_misc import load_knowledge_index


@pytest.fixture
def repo_root():
    """Zwraca ścieżkę do katalogu głównego repozytorium Ishkarim."""
    root = Path(__file__).resolve().parents[3]  # projects/ishkarim-misc/tests/ → repo root
...
class TestModuleIntegrity:
    ...
    def test_version_format(self):
        assert re.match(r"\d+\.\d+\.\d+", ishkarim_misc.__version__)

    def test_modules_all_strings(self):
        for name in ishkarim_misc.MODULES:
