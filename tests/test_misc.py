"""
test_misc.py — testy dla modułu ishkarim_misc.
"""
import pytest
...
class TestImport:
    ...
    def test_import_main(self):
        ...
        assert ishkarim_misc.__version__ == "0.1.0"

    def test_area(self):
        import ishkarim_misc
        assert ishkarim_misc.__area__ == "misc"

    def test_modules_list_nonempty(self):
        import ishkarim_misc
        assert len(ishkarim_misc.MODULES) > 0

    def test_submodule_imports(self):
        from ishkarim_misc import utils
...
class TestKnowledgeIndex:
    ...
    def test_returns_list(self, tmp_path):
        import ishkarim_misc
        result = ishkarim_misc.load_knowledge_index(root=str(tmp_path))
        assert isinstance(result, list)

    def test_record_keys(self, tmp_path):
        import os, ishkarim_misc
        # create a minimal fake source dir
        src_dir = tmp_path / ishkarim_misc.MODULES[0]
...
class TestUtils:
    ...
    def test_read_work_md_missing(self, tmp_path):
        from ishkarim_misc.utils import read_work_md
    ...
    def test_extract_python_blocks(self):
        from ishkarim_misc.utils import extract_python_blocks
        md = """```python
def greet():
    return "hello world"
